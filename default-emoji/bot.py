...
def get_main_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="🔄️ Refresh", callback_data='menu_main'),
            InlineKeyboardButton(text="🧰 Tools", callback_data='menu_tools')
        ],
        [
            InlineKeyboardButton(text="🐳 Docker", callback_data='menu_docker'),
            InlineKeyboardButton(text="🧲 qBitT", callback_data='menu_qbit')
        ],
        [
            InlineKeyboardButton(text="🔝 Processes", callback_data='menu_processes'),
...
class SystemMonitor:
    ...
    def get_updates_info() -> str:
        try:
            # 1. Update package lists (requires sudo)
            ...
            if count == 0:
                return "<b>✅ System is up to date.</b>"
            
            msg = f"<b>📦 Updates Available:</b> {count}\n━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            ...
            return f"⚠️ Error checking updates: {str(e)[:50]}"
    ...
    def get_stats() -> str:
        ...
        try:
            ...
            return "⚠️ Error retrieving system stats"
    ...
    def get_docker_stats() -> str:
        try:
            ...
            if not containers:
                return "<b>🐳 Docker:</b> No containers found."
        except Exception as e:
            return f"⚠️ Docker API Error: {str(e)[:50]}"
    ...
    def run_speedtest() -> str:
        if not conf.SPEEDTEST_ENABLED:
            return "⚠️ Speedtest is disabled in config"
        ...
        try:
            ...
            if result.returncode != 0:
                return f"⚠️ Speedtest Error: {result.stderr[:100]}"
            ...
            msg = (
                ...
                f"━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                f"⬇️ <b> Download:</b> {dl_mbps:.1f} Mbps\n"
                f"⬆️ <b> Upload:</b> {ul_mbps:.1f} Mbps\n"
            ...
            return "⚠️ Speedtest timed out (>60s)"
        except Exception as e:
            return f"⚠️ Speedtest Error: {str(e)[:50]}"

    @staticmethod
    def get_power_detail() -> str:
        if not conf.SHELLY_IP:
            return "⚠️ Shelly IP not configured"
        try:
            resp = requests.get(
                f"http://{conf.SHELLY_IP}/rpc/Switch.GetStatus?id=0",
                timeout=conf.REQUEST_TIMEOUT
    ...
    def get_process_info() -> str:
        try:
            ...
            return "⚠️ Error retrieving process info"
...
class AdminMiddleware(BaseMiddleware):
    async def __call__(
        ...
        if not user or user.id != conf.ADMIN_ID:
            if user:
                logger.warning(f"Unauthorized access attempt: {user.first_name} ({user.id})")
            return
...
async def handle_modules(query: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔄️ Refresh", callback_data=data)],
...
async def handle_menu_system(query: CallbackQuery):
    ...
    text = (
        ...
        f"━━━━━━━━━━━━━━━━━━━━━━━━━\n"        
        f"<i>⚠️ Handle with care</i>"
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="🔄️ Reboot", callback_data='confirm_reboot'),
            InlineKeyboardButton(text="🛑 Shutdown", callback_data='confirm_shutdown')
        ],
        [InlineKeyboardButton(text="↩️ Back", callback_data='menu_main', style='primary')]
    ])
    await safe_edit_message(query, text, kb)
...
async def handle_menu_tools(query: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="🌐 Network", callback_data='tool_network'),
            InlineKeyboardButton(text="⚡ Power", callback_data='tool_power')
        ],
        [
            InlineKeyboardButton(text="🚀 Speedtest", callback_data='tool_speed'),
            InlineKeyboardButton(text="🔐 Logins", callback_data='tool_logins')
        ],
        [            
            InlineKeyboardButton(text="📦 Updates", callback_data='tool_updates'),
            InlineKeyboardButton(text="🧹 Cleanup", callback_data='confirm_cleanup')
        ],
        [
            InlineKeyboardButton(text="↩️ Back", callback_data='menu_main', style='primary')
        ]
...
async def handle_tools(query: CallbackQuery):
    ...
    if data == 'tool_network':
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="↩️ Back", callback_data='menu_tools', style='primary')]])
        
    elif data == 'tool_power':
        text = await run_blocking(SystemMonitor.get_power_detail)
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔄 Refresh", callback_data='tool_power')],
            [InlineKeyboardButton(text="↩️ Back", callback_data='menu_tools', style='primary')]
        ])
        
    elif data == 'tool_updates':
        await safe_edit_message(query, "⏳ <b>Checking repositories...</b>", None)
        text = await run_blocking(SystemMonitor.get_updates_info)
        
        # Determine buttons based on result
        has_updates = "Updates Available" in text
        
        buttons = []
        if has_updates:
            buttons.append([InlineKeyboardButton(text="✅ Upgrade All", callback_data='confirm_upgrade', style='success')])
        
        buttons.append([InlineKeyboardButton(text="↩️ Back", callback_data='menu_tools', style='primary')])
        ...
        text = f"<b>🔑 Recent Logins </b>\n\n<pre>{result.stdout}</pre>"
        kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="↩️ Back", callback_data='menu_tools', style='primary')]])
        
    elif data == 'tool_speed':
        await safe_edit_message(query, "<b>⏳ Running Speedtest.</b>\n<i>This may take a while</i>", None)
...
async def handle_confirmations(query: CallbackQuery):
    ...
    confirm_map = {
        'reboot': ("<b>⚠️ CONFIRM REBOOT</b>", 'menu_system'),
        'shutdown': ("<b>⚠️ CONFIRM SHUTDOWN</b>", 'menu_system'),
        'upgrade': ("<b>⚠️ CONFIRM SYSTEM UPGRADE</b>\n\nThis will run <code>apt-get upgrade -y</code>.\nBot may be unresponsive during process.", 'tool_updates'),
        'cleanup': ("<b>⚠️ CONFIRM CLEANUP</b>\n<i>Runs apt autoremove & clean</i>", 'menu_tools')
    }
    ...
    if action in confirm_map:
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [
                InlineKeyboardButton(text="✅ Confirm", callback_data=f'do_{action}', style='success'),
                InlineKeyboardButton(text="❌ Cancel", callback_data=back, style='danger')
            ]
        ])
        await safe_edit_message(query, text, kb)
...
async def handle_actions(query: CallbackQuery):
    ...
    if data == 'do_cleanup':
        await safe_edit_message(query, "🧹 <b>Running cleanup...</b>", None)
        try:
            ...
            text = f"<b>⚠️ Cleanup Failed</b>\n<pre>{str(e)[:100]}</pre>"
        kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="↩️ Back", callback_data='menu_tools', style='primary')]])
        await safe_edit_message(query, text, kb)
    
    elif data == 'do_upgrade':
        await safe_edit_message(query, "<b>📦 Upgrading System...</b>\n<i>This may take a while. Do not turn off.</i>", None)
        ...
        try:
            ...
            text = "<b>✅ Upgrade Complete</b>"
        except Exception as e:
            text = f"<b>⚠️ Upgrade Failed</b>\n<pre>{str(e)[:100]}</pre>"
            
        kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="↩️ Back", callback_data='menu_tools', style='primary')]])
        await safe_edit_message(query, text, kb)

    elif data == 'do_reboot':        
        await safe_edit_message(query, "<b>🔄 System Rebooting...</b>", None)
        await run_blocking(subprocess.run, ["sudo", "/usr/sbin/reboot"])
        
    elif data == 'do_shutdown':
        await safe_edit_message(query, "<b>🛑 System Shutting Down...</b>", None)
